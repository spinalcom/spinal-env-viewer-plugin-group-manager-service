...
export default class GroupManagerService {
  ...
  public async createGroupContext(
    contextName: string,
    childrenType: string,
    ...
    let contextFound = contexts.find(
      (context) => context.name.get() === contextName
    );

    if (typeof contextFound !== 'undefined')
      return Promise.resolve(
        SpinalGraphService.getRealNode(contextFound.id.get())
      );

    return SpinalGraphService.addContext(
      contextName,
      `${childrenType}${CONTEXTGROUP_TYPE_END}`,
      new Model({
        name: contextName,
  ...
  public async linkElementToGroup(
    ...
    spinalEventEmitter.emit(ELEMENT_LINKED_TO_GROUP_EVENT, {
      groupId,
      elementId,
    });
    return result;
  ...
  public unLinkElementToGroup(
    ...
    return spinalGroup
      ...
      .then((result) => {
        spinalEventEmitter.emit(ELEMENT_UNLINKED_TO_GROUP_EVENT, {
          groupId,
          elementId,
        });
        return result;
  ...
  public getGroupCategory(groupId: string): Promise<SpinalNodeRef | undefined> {
  ...
  public isRoomGroupContext(type: string): boolean {
    return (
      type ==
        `${geographicService.constants.ROOM_TYPE}${CONTEXTGROUP_TYPE_END}` ||
      OLD_CONTEXTS_TYPES.ROOMS_GROUP_CONTEXT == type
    );
  }
  ...
  public isEquipmentGroupContext(type: string): boolean {
    return (
      type ==
        `${geographicService.constants.EQUIPMENT_TYPE}${CONTEXTGROUP_TYPE_END}` ||
      OLD_CONTEXTS_TYPES.EQUIPMENTS_GROUP_CONTEXT == type
    );
  }
  ...
  public isRoomsGroup(type: string): boolean {
    return (
      type == `${geographicService.constants.ROOM_TYPE}${GROUP_TYPE_END}` ||
      OLD_CONTEXTS_TYPES.ROOMS_GROUP_CONTEXT.replace('Context', '') == type ||
      type === OLD_GROUPS_TYPES.ROOMS_GROUP
    );
  }
  ...
  public isEquipementGroup(type: string): boolean {
    return (
      type ==
        `${geographicService.constants.EQUIPMENT_TYPE}${GROUP_TYPE_END}` ||
      OLD_CONTEXTS_TYPES.EQUIPMENTS_GROUP_CONTEXT.replace('Context', '') ==
        type ||
      type === OLD_GROUPS_TYPES.EQUIPMENTS_GROUP
    );
  }
  ...
  public getChildrenType(type: string): string | undefined {
  ...
  private _getOldTypes(type: string): string | undefined {
    switch (type) {
